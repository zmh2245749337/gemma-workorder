"""Fine-tune Gemma for the constrained service-query JSON contract.

Direct successor to train_workorder_qlora.py -- identical training loop,
pointed at the CrossWOZ-derived service-query task instead. Run this on a
CUDA GPU (Colab T4 or a local NVIDIA GPU both work). It trains only LoRA
adapters; model weights and dataset stay local to wherever the script runs
...
from gemma_eval.service_query_data import canonical_json, load_jsonl, service_query_prompt


def encode_example(tokenizer, row: dict, max_length: int) -> dict[str, list[int]]:
    prompt_encoded = tokenizer.apply_chat_template(
        [{"role": "user", "content": service_query_prompt(row["input_text"])}],
        tokenize=True,
...
class ServiceQueryDataset(torch.utils.data.Dataset):
...
def main() -> None:
    parser = argparse.ArgumentParser(description="QLoRA fine-tuning for the service-query task")
    parser.add_argument("--model-id", default="google/gemma-3-1b-it")
    parser.add_argument("--train", type=Path, default=Path("data/service_query/train.jsonl"))
    parser.add_argument("--validation", type=Path, default=Path("data/service_query/validation.jsonl"))
    parser.add_argument("--output-dir", type=Path, default=Path("artifacts/service_query_qlora_adapter"))
    ...
    model = AutoModelForCausalLM.from_pretrained(args.model_id, token=hf_token, device_map="auto", quantization_config=quantization)
    ...
    train_dataset = ServiceQueryDataset(tokenizer, train_rows, args.max_length)
    validation_dataset = ServiceQueryDataset(tokenizer, validation_rows, args.max_length)
    ...
    metadata = {
        "scope": "CrossWOZ-derived service-query extraction (real, human-annotated Chinese dialogue data; next_action routing is this project's own design).",
